...
const VoteLine = ({ vote, i, reloadVotes }) => {
	...
	const onConfirmClick = async requestId => {
		...
		setTimeout(async () => {
			...
			if (removedVote.error) return setUnvoteMsg(removedVote.error)
	...
	return (
		<li key={i}>
			{unvoteMsg === '' && (
				<>
					<span className='user-votes-spans' onClick={onUnVoteClick}>
						Un-Vote
					</span>
					...
					<span className='user-votes-spans city'>{vote.city}</span>
			...
			{unvoteMsg === 'confirm' && (
				<div className='confirm-remove'>
					...
					<FontAwesomeIcon
						icon={faCheck}
						onClick={() =>
							onConfirmClick(vote.request_id, vote.user_id)
						}
						color='#28A745'
					/>
					<FontAwesomeIcon
			...
			{unvoteMsg && unvoteMsg !== 'confirm' && (
				<span className='user-votes-spans'>{unvoteMsg}</span>
			)}
		</li>
	)
}
